...
dataset_dir = r'C:\DataScience\deepdaiv_emergencyclf\위급상황 음성_음향\Training'
#r'/home/sholee123/Space/DataScience/deepdaiv_emergencyclf/위급상황 음성_음향/Training'
#r'C:\DataScience\deepdaiv_emergencyclf\위급상황 음성_음향\Training'
dataset_dir_test = r'C:\DataScience\deepdaiv_emergencyclf\위급상황 음성_음향\Validation'
#r'/home/sholee123/Space/DataScience/deepdaiv_emergencyclf/위급상황 음성_음향/Validation'
#r'C:\DataScience\deepdaiv_emergencyclf\위급상황 음성_음향\Validation'
...
dataset_config = {
    'meta_csv': os.path.join(dataset_dir, "custom_meta.csv"),
    'audio_path': os.path.join(dataset_dir),
    'meta_csv_test': os.path.join(dataset_dir_test, "custom_meta_test.csv"),
    'audio_path_test': os.path.join(dataset_dir_test),
    'num_of_classes': 12
}
...
class AudioSetDataset(TorchDataset):
    def __init__(self, meta_csv, audiopath, fold, train=False, resample_rate=32000, classes_num=12,
                 clip_length=8, gain_augment=0):
        """
        ...
        self.folder_list = os.listdir(self.audiopath)
        self.wav_list = [s for s in self.folder_list if '원천' in s]
    ...
    def __getitem__(self, index):
        ...
        sub_path = [x for x in self.wav_list if key in x][0] + '/'

        waveform, _ = librosa.load(self.audiopath +'/'+ sub_path + row.filename, sr=self.resample_rate, mono=True)
...
def get_base_test_set(resample_rate=32000, fold=1):
    meta_csv = dataset_config['meta_csv_test']
    audiopath = dataset_config['audio_path_test']
